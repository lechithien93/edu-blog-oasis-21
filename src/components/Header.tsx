...
const Header = ({ showBanner = false }: HeaderProps) => {
  ...
  return (
    <>
      <header className={`fixed top-0 left-0 right-0 z-50 bg-white shadow-md ${showBanner ? 'h-[500px]' : 'h-auto'} mb-${showBanner ? '16' : '0'}`}>
        {showBanner && (
      ...
      <div className={`${showBanner ? 'h-[500px]' : 'h-20'}`}></div>
    </>
  );
};
